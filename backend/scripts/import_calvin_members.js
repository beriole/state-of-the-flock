/**
 * Script d'importation des membres pour le Gouverneur Calvin
 * Usage: node backend/scripts/import_calvin_members.js
 * Pour exécuter réellement: DRY_RUN=false node backend/scripts/import_calvin_members.js
 */

const path = require('path');
require('dotenv').config({ path: path.join(__dirname, '../.env') });
const { User, Member, setupAssociations, sequelize } = require('../models');

// Configuration
const DRY_RUN = process.env.DRY_RUN !== 'false'; // Par défaut, on fait un test sans écrire
const GOVERNOR_EMAIL = 'calvin.rev@njangui.org';

async function importMembers() {
  try {
    console.log('--- 🚀 DÉBUT DE L\'IMPORTATION DES MEMBRES ---');
    console.log(`📡 Mode: ${DRY_RUN ? '🔍 TEST (DRY_RUN)' : '💾 RÉEL (ÉCRITURE)'}`);
    ...
    if (!calvin) {
      console.error(`❌ Erreur: Gouverneur avec l'email ${GOVERNOR_EMAIL} introuvable.`);
      console.log('💡 Assurez-vous que l\'utilisateur existe dans la base de données.');
      process.exit(1);
    }

    console.log(`👤 Gouverneur trouvé: ${calvin.first_name} ${calvin.last_name}`);
    console.log(`🆔 ID: ${calvin.id}`);
    console.log(`📍 Area ID: ${calvin.area_id || 'NON DÉFINI (⚠️ Attention: obligaroire pour les membres)'}`);

    if (!calvin.area_id) {
       console.error('❌ Erreur critique: Le gouverneur n\'a pas d\'area_id. L\'importation va échouer car ce champ est obligatoire pour les membres.');
       process.exit(1);
    }

    // 2. Préparer les données (Regroupées par catégories du message utilisateur)
    const rawData = [
      // SHEEP
    ...
    console.log(`📊 Total membres à importer: ${rawData.length}`);

    // Transformation pour la DB
    const membersToCreate = rawData.map(item => {
      const nameParts = item.name.split(' ');
      const firstName = nameParts[0];
      const lastName = nameParts.slice(1).join(' ') || '.'; // '.' si pas de nom de famille
      
      return {
        first_name: firstName,
        last_name: lastName,
        phone_primary: item.phone,
        gender: 'M', // Spécifié comme Homme par défaut
        state: item.state,
        area_id: calvin.area_id,
        leader_id: calvin.id,
        notes: `Importé via script. Catégorie: ${item.category}${item.incomplet ? ' (Numéro incomplet)' : ''}`,
        is_active: true,
        is_registered: false
      };
    });
    ...
    if (DRY_RUN) {
      console.log('\n--- 📋 APERÇU DES DONNÉES (DRY_RUN) ---');
      console.log('Voici les 5 premiers membres à titre d\'exemple:');
      console.log(membersToCreate.slice(0, 5));
      console.log(`... et ${membersToCreate.length - 5} autres.`);
      console.log('\n✅ Fin de l\'aperçu. Aucune donnée n\'a été écrite.');
      console.log('💡 Pour exécuter réellement, relancez avec: DRY_RUN=false node backend/scripts/import_calvin_members.js');
    } else {
      ...
      for (const memberData of membersToCreate) {
        try {
          ...
          process.stdout.write('.'); // Petit indicateur de progression
          successCount++;
        } catch (err) {
          console.error(`\n❌ Erreur pour ${memberData.first_name} ${memberData.last_name}: ${err.message}`);
          errorCount++;
        }
      }
