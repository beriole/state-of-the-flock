...
// Route de correction pour Noella
app.get('/api/fix/noella', async (req, res) => {
  try {
    const { User, Region, Area } = require('./models');
    const GOVERNOR_EMAIL = 'noella@njangui.org';
    
    const user = await User.findOne({ where: { email: GOVERNOR_EMAIL } });
    if (!user) return res.json({ error: 'Noella introuvable' });

    // 1. Trouver une région sans gouverneur ou en créer une
    let region = await Region.findOne({ where: { governor_id: null, name: 'Région 4' } });
    if (!region) {
    ...
    // 2. S'assurer que la zone de Noella appartient à cette région
    ...
    res.json({
      message: 'Correction effectuée pour Noella',
      user_id: user.id,
      area_id: user.area_id,
...
// Import Noella
const { importNoellaMembers } = require('./utils/importNoella');
app.get('/api/members/seed-noella', async (req, res) => {
  try {
     const { User } = require('./models');
     const user = await User.findOne({ where: { email: 'noella@njangui.org' } });
     if (!user) return res.status(404).json({ error: 'User not found' });
     
     const result = await importNoellaMembers(user.id, user.area_id);
     res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/members/cleanup-noella', async (req, res) => {
  try {
...
// Route de correction pour Akongnwi
app.get('/api/fix/akongnwi', async (req, res) => {
  try {
    const { User, Region, Area } = require('./models');
    const GOVERNOR_EMAIL = 'akongnwi@njangui.org';
    
    const user = await User.findOne({ where: { email: GOVERNOR_EMAIL } });
    if (!user) return res.json({ error: 'Akongnwi introuvable' });

    // Trouver une région sans gouverneur ou en créer une
    let region = await Region.findOne({ where: { governor_id: null, name: 'Région 5' } });
    if (!region) {
       region = await Region.findOne({ where: { governor_id: null } });
    }
    ...
    if (!region) {
      region = await Region.create({
        name: 'Région Gouverneur Akongnwi',
        governor_id: user.id
      });
    } else {
      await region.update({ governor_id: user.id });
    ...
    // S'assurer que la zone de Akongnwi appartient à cette région
    const area = await Area.findByPk(user.area_id);
    if (area) {
      await area.update({ region_id: region.id });
    }
    ...
    res.json({
      message: 'Correction effectuée pour Akongnwi',
      user_id: user.id,
      area_id: user.area_id,
      assigned_region: region.name,
      area_linked: area ? area.name : 'Aucune zone trouvée'
...
// Import Akongnwi
const { importAkongnwiMembers } = require('./utils/importAkongnwi');
app.get('/api/members/seed-akongnwi', async (req, res) => {
  try {
     const { User } = require('./models');
     const user = await User.findOne({ where: { email: 'akongnwi@njangui.org' } });
     if (!user) return res.status(404).json({ error: 'User not found' });
     
     const result = await importAkongnwiMembers(user.id, user.area_id);
     res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/members/cleanup-akongnwi', async (req, res) => {
  try {
    const { Member, User } = require('./models');
    const user = await User.findOne({ where: { email: 'akongnwi@njangui.org' } });
    if (!user) return res.status(404).json({ error: 'User not found' });
...
// Route de correction pour Rayon
app.get('/api/fix/rayon', async (req, res) => {
  try {
    const { User, Region, Area } = require('./models');
    const GOVERNOR_EMAIL = 'rayon@njangui.org';
    
    const user = await User.findOne({ where: { email: GOVERNOR_EMAIL } });
    if (!user) return res.json({ error: 'Rayon introuvable' });

    // Trouver une région sans gouverneur ou en créer une
    let region = await Region.findOne({ where: { governor_id: null, name: 'Région 6' } });
    if (!region) {
       region = await Region.findOne({ where: { governor_id: null } });
    }
    ...
    // S'assurer que la zone de Rayon appartient à cette région
    const area = await Area.findByPk(user.area_id);
    if (area) {
      await area.update({ region_id: region.id });
    }
    ...
    res.json({
      message: 'Correction effectuée pour Rayon',
      user_id: user.id,
      area_id: user.area_id,
      assigned_region: region.name,
      area_linked: area ? area.name : 'Aucune zone trouvée'
...
// Import Rayon
const { importRayonMembers } = require('./utils/importRayon');
app.get('/api/members/seed-rayon', async (req, res) => {
  try {
     const { User } = require('./models');
     const user = await User.findOne({ where: { email: 'rayon@njangui.org' } });
     if (!user) return res.status(404).json({ error: 'User not found' });
     
     const result = await importRayonMembers(user.id, user.area_id);
     res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/members/cleanup-rayon', async (req, res) => {
  try {
    const { Member, User } = require('./models');
    const user = await User.findOne({ where: { email: 'rayon@njangui.org' } });
    if (!user) return res.status(404).json({ error: 'User not found' });
...
app.get('/api/fix/clovis', async (req, res) => {
  try {
    ...
    // Trouver une région sans gouverneur ou en créer une
    let region = await Region.findOne({ where: { governor_id: null, name: 'Région 8' } });
    if (!region) {
       region = await Region.findOne({ where: { governor_id: null } });
    }
    ...
    // S'assurer que la zone de Clovis appartient à cette région
    const area = await Area.findByPk(user.area_id);
    if (area) {
      await area.update({ region_id: region.id });
    }
