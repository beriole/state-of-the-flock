...
const { importRosaMembers } = require('./utils/importRosa');
app.get('/api/members/seed-rosa', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'rosa.joy@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Rosa@123', 10);
      user = await User.create({
        first_name: 'Rosa',
        last_name: 'Joy',
    ...
    // EXTREMELY IMPORTANT: Force the assigned area back to the user row to prevent the dashboard glitch!
    if (!user.area_id || user.area_id !== targetAreaId) {
      await user.update({ area_id: targetAreaId });
    }

    const result = await importRosaMembers(user.id, targetAreaId);
...
const { importDexterV2Members } = require('./utils/importDexterV2');
app.get('/api/members/seed-dexter-v2', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'dexter.ps@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Dexter@123', 10);
      user = await User.create({
        first_name: 'Dexter',
        last_name: 'Leader',
    ...
    const result = await importDexterV2Members(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importYvetteMembers } = require('./utils/importYvette');
app.get('/api/members/seed-yvette', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'yvette.lp@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Yvette@123', 10);
      user = await User.create({
        first_name: 'Yvette',
        last_name: 'Leader',
    ...
    const result = await importYvetteMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importLydiaMembers } = require('./utils/importLydia');
app.get('/api/members/seed-lydia', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'lydia.lp@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Lydia@123', 10);
      user = await User.create({
        first_name: 'Lydia',
        last_name: 'Leader',
    ...
    const result = await importLydiaMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/members/seed-reward', async (req, res) => {
  try {
    ...
    let user = await User.findOne({ where: { email: 'reward@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Reward@123', 10);
      user = await User.create({
        first_name: 'Reward',
        last_name: 'Leader',
    ...
    const result = await importRewardMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importTraceyMembers } = require('./utils/importTracey');
app.get('/api/members/seed-tracey', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'tracey.rev@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Tracey@123', 10);
      user = await User.create({
        first_name: 'Tracey',
        last_name: 'Leader',
    ...
    const result = await importTraceyMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importShalomMembers } = require('./utils/importShalom');
app.get('/api/members/seed-shalom', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'shalom@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Shalom@123', 10);
      user = await User.create({
        first_name: 'Shalom',
        last_name: 'Leader',
    ...
    const result = await importShalomMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importMikeMembers } = require('./utils/importMike');
app.get('/api/members/seed-mike', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'mike.ps@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Mike@123', 10);
      user = await User.create({
        first_name: 'Mike',
        last_name: 'Leader',
    ...
    const result = await importMikeMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/members/seed-clifford', async (req, res) => {
  try {
    ...
    let user = await User.findOne({ where: { email: 'clifford@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Clifford@123', 10);
      user = await User.create({
        first_name: 'Clifford',
        last_name: 'Leader',
    ...
    const result = await importCliffordMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importDexterGESMembers } = require('./utils/importDexterGES');
app.get('/api/members/seed-dexter-ges', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    const result = await importDexterGESMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importGesAirportMembers } = require('./utils/importGesAirport');
app.get('/api/members/seed-ges-airport', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'ges.airport@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Ges@123', 10);
      user = await User.create({
        first_name: 'GES Airport',
        last_name: 'Leader',
    ...
    const result = await importGesAirportMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importElyseeMembers } = require('./utils/importElysee');
app.get('/api/members/seed-elysee', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'elysee.dr@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Elysee@123', 10);
      user = await User.create({
        first_name: 'Elysee',
        last_name: 'Leader',
    ...
    const result = await importElyseeMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importLydiaJESMembers } = require('./utils/importLydiaJES');
app.get('/api/members/seed-lydia-jes', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    const result = await importLydiaJESMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importYvetteJESMembers } = require('./utils/importYvetteJES');
app.get('/api/members/seed-yvette-jes', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    const result = await importYvetteJESMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const { importSamuelJESMembers } = require('./utils/importSamuelJES');
app.get('/api/members/seed-samuel-jes', async (req, res) => {
  try {
    const { User, Area, Region } = require('./models');
    const bcrypt = require('bcrypt');
    ...
    let user = await User.findOne({ where: { email: 'samuel.ps@njangui.org' } });
    let created = false;
    
    if (!user) {
      const hashedPassword = await bcrypt.hash('Samuel@123', 10);
      user = await User.create({
        first_name: 'Samuel',
        last_name: 'Leader',
    ...
    const result = await importSamuelJESMembers(user.id, targetAreaId);
    res.json({ userCreated: created, forcedAreaSync: true, importResult: result });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
